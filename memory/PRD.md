...
- Registration notification to the `ADMIN_NOTIFICATION_EMAIL` recipient via SendGrid
...
## Test Configuration
- Live test credentials are supplied through `BESIDE_TEST_*` environment variables.
- No reusable test or admin credentials are stored in the repository.

## API Keys & Integrations
- **Stripe:** existing integration reads `STRIPE_API_KEY` when configured
- **Gemini:** reads `GEMINI_API_KEY` (or the legacy `EMERGENT_LLM_KEY`) when configured
- **SendGrid:** reads `SENDGRID_API_KEY`, `SENDER_EMAIL`, and `ADMIN_NOTIFICATION_EMAIL`
