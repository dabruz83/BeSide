...
BASE_URL = os.environ.get('BACKEND_TEST_URL', '').rstrip('/')

# Integration-test credentials must be supplied explicitly.
TEST_USER_EMAIL = os.environ.get('BESIDE_TEST_USER_EMAIL')
TEST_USER_PASSWORD = os.environ.get('BESIDE_TEST_USER_PASSWORD')
ADMIN_EMAIL = os.environ.get('BESIDE_TEST_ADMIN_EMAIL')
ADMIN_PASSWORD = os.environ.get('BESIDE_TEST_ADMIN_PASSWORD')
REGISTRATION_PASSWORD = f"Test-{uuid.uuid4().hex}-A1!"
INVALID_PASSWORD = f"Invalid-{uuid.uuid4().hex}"

pytestmark = pytest.mark.skipif(
    not all([BASE_URL, TEST_USER_EMAIL, TEST_USER_PASSWORD, ADMIN_EMAIL, ADMIN_PASSWORD]),
    reason="Live integration tests require BACKEND_TEST_URL and BESIDE_TEST_* credentials",
)
...
class TestAuthentication:
    ...
    def test_register_with_first_name(self):
        ...
        login_resp = requests.post(f"{BASE_URL}/api/auth/login", json={
            "email": unique_email, "password": REGISTRATION_PASSWORD
        })
