...
class BESIDEAPITester:
    def __init__(self, base_url=None):
        configured_url = base_url or os.environ.get("BACKEND_TEST_URL", "http://localhost:8000")
...
def main():
    ...
    test_password = f"Test-{uuid.uuid4().hex}-A1!"
    
    # Register user
    reg_success, reg_response = tester.test_user_registration(test_email, test_password)
    if not reg_success:
        print("❌ Registration failed, stopping authenticated tests")
